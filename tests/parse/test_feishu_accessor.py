...
from openviking.parse.feishu.converter import FeishuConvertOptions
...
def test_fetch_all_blocks_uses_user_access_token_option(monkeypatch):
    ...
    accessor._converter._user_token_client = SimpleNamespace(
        docx=SimpleNamespace(v1=SimpleNamespace(document_block=SimpleNamespace(list=list_blocks)))
    )

    blocks = accessor._converter._fetch_all_blocks(
        "doc_token",
        options=FeishuConvertOptions(feishu_access_token="u-test"),
    )
...
def test_resolve_image_refs_respects_download_images_disabled():
    accessor = FeishuAccessor()
    accessor._converter._config = SimpleNamespace(download_images=False)
...
def test_resolve_image_refs_downloads_media_and_rewrites_markdown(monkeypatch):
    ...
    accessor = FeishuAccessor()
    accessor._converter._config = SimpleNamespace(download_images=True)
    accessor._converter._client = SimpleNamespace(request=request_media)
